...
void register_core_objects() {
	/* object */
	{
		...
		object_variable_t vars[] = { { .name = hashed_string_generate("type"), .type = variant_type_u64, .offset = 0 },
									 { 0, 0, 0 } };

		variables = TKP_MALLOC(sizeof(vars));
		memcpy(variables, vars, sizeof(vars));

		object_definition_t def = { .name = hashed_string_generate("object"),
	...
	/* component */
	{
		...
		object_variable_t vars[] = { { .name = hashed_string_generate("parent"),
		...
		object_definition_t def = { .name = hashed_string_generate("component"),
	...
	/* resource */
	{
		...
		object_variable_t vars[] = { { .name = hashed_string_generate("path"),
		...
		object_definition_t def = { .name = hashed_string_generate("resource"),
	...
	/* entity */
	{
		...
		object_variable_t vars[] = {
			{ .name = hashed_string_generate("name"), .type = variant_type_string, .offset = offsetof(entity_t, name) },
			{ .name = hashed_string_generate("id"), .type = variant_type_u64, .offset = offsetof(entity_t, id) },
			{ 0, 0, 0 }
		};
		...
		object_definition_t def = { .name = hashed_string_generate("entity"),
