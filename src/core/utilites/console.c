...
/*
 *	what i want to do in the future is wait until the end of the frame, and
 * then write to the log file, kinda as a batching system.
 *
 *	i could extend this further and do it every 5 or so frames, but that
 * kinda gets rid of the point of having a log system that can be as up-to-date
 * as possible (i consider waiting until the end of the frame as up-to-date)
 */

static FILE* _log_file = NULL;
static FILE* _console = NULL;

static const string _help_message = "-h/--help\t-\tprints this help message\n"
									"--width\t\t-\tspecify custom window width\n"
									"--height\t-\tspecify custom window height\n"
									"--log-path\t-\tspecify custom log path (default is "
									"~/.cache/tkpengine/logs/log.txt)\n"
