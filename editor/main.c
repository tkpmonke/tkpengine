...
#include "display/rendering/backends/opengl_es2/opengl_es2_rendering_interface.h"
...
int main(int argc, string* argv) {
	...
	window_t root_window = { .rect.position[0] = 0,
							 .rect.position[1] = 0,
...
#else
	rendering_interface_t* rendering_interface = opengl_es2_create_rendering_interface();
#endif
	rendering_interface_set(rendering_interface);
	rendering_interface->init(&root_window);

	rendering_shader_t vertex =
		rendering_interface->compile_shader((string)vertex_shader_source, rendering_shader_vertex);
	rendering_shader_t fragment =
		rendering_interface->compile_shader((string)fragment_shader_source, rendering_shader_fragment);
	rendering_pipeline_t pipeline = rendering_interface->create_graphics_pipeline(vertex, fragment);

	rendering_buffer_t vertex_buffer = rendering_interface->create_buffer(
		sizeof(vertices), rendering_buffer_vertex | rendering_buffer_write, (void*)vertices);
	rendering_buffer_t index_buffer = rendering_interface->create_buffer(
		sizeof(indices), rendering_buffer_index | rendering_buffer_write, (void*)indices);

	vertex_attribute_object vao = rendering_interface->create_vertex_attribute_object(
		(vertex_attribute_t[]){
			(vertex_attribute_t){ .offset = 0, .size = 3, .stride = 3 * sizeof(f32), .type = variant_type_f32 },
		},
